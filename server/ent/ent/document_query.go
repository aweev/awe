...
type DocumentQuery struct {
	...
	withCategory *DocumentCategoryQuery
	withFKs      bool
...
func (_q *DocumentQuery) QueryUploader() *UserQuery {
...
func (_q *DocumentQuery) QueryCategory() *DocumentCategoryQuery {
...
func (_q *DocumentQuery) WithUploader(opts ...func(*UserQuery)) *DocumentQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *DocumentQuery) WithCategory(opts ...func(*DocumentCategoryQuery)) *DocumentQuery {
	query := (&DocumentCategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if _q.withUploader != nil || _q.withCategory != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withUploader; query != nil {
		if err := _q.loadUploader(ctx, query, nodes, nil,
			func(n *Document, e *User) { n.Edges.Uploader = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCategory; query != nil {
		if err := _q.loadCategory(ctx, query, nodes, nil,
			func(n *Document, e *DocumentCategory) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadUploader(ctx context.Context, query *UserQuery, nodes []*Document, init func(*Document), assign func(*Document, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Document)
	for i := range nodes {
		if nodes[i].user_document_uploads == nil {
			continue
		}
		fk := *nodes[i].user_document_uploads
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_document_uploads" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DocumentQuery) loadCategory(ctx context.Context, query *DocumentCategoryQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentCategory)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Document)
	for i := range nodes {
		if nodes[i].document_category_documents == nil {
			continue
		}
		fk := *nodes[i].document_category_documents
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "document_category_documents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
