...
type OrganizationQuery struct {
	...
	withPartnerships *PartnershipQuery
...
// QueryPartnerships chains the current query on the "partnerships" edge.
func (_q *OrganizationQuery) QueryPartnerships() *PartnershipQuery {
...
func (_q *OrganizationQuery) QueryGrants() *GrantQuery {
...
func (_q *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withPartnerships: _q.withPartnerships.Clone(),
...
// WithPartnerships tells the query-builder to eager-load the nodes that are connected to
// the "partnerships" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithPartnerships(opts ...func(*PartnershipQuery)) *OrganizationQuery {
	query := (&PartnershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPartnerships = query
	return _q
}
...
func (_q *OrganizationQuery) WithGrants(opts ...func(*GrantQuery)) *OrganizationQuery {
	query := (&GrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withPartnerships; query != nil {
		if err := _q.loadPartnerships(ctx, query, nodes,
			func(n *Organization) { n.Edges.Partnerships = []*Partnership{} },
			func(n *Organization, e *Partnership) { n.Edges.Partnerships = append(n.Edges.Partnerships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGrants; query != nil {
		if err := _q.loadGrants(ctx, query, nodes,
			func(n *Organization) { n.Edges.Grants = []*Grant{} },
			func(n *Organization, e *Grant) { n.Edges.Grants = append(n.Edges.Grants, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) loadPartnerships(ctx context.Context, query *PartnershipQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Partnership)) error {
	...
	query.Where(predicate.Partnership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.PartnershipsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.organization_partnerships
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_partnerships" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_partnerships" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *OrganizationQuery) loadGrants(ctx context.Context, query *GrantQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Grant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Grant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.GrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_grants
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_grants" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_grants" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
