...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if _q.withConversation != nil || _q.withSender != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *MessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	...
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].user_messages_sent == nil {
			continue
		}
		fk := *nodes[i].user_messages_sent
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_messages_sent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
