...
//		GroupBy(rolepermission.FieldRole).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.RolePermission.Query().
//		Select(rolepermission.FieldRole).
//		Scan(ctx, &v)
...
func (_q *RolePermissionQuery) loadPermission(ctx context.Context, query *PermissionQuery, nodes []*RolePermission, init func(*RolePermission), assign func(*RolePermission, *Permission)) error {
	...
	for i := range nodes {
		if nodes[i].permission_roles == nil {
			continue
		}
		fk := *nodes[i].permission_roles
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "permission_roles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
