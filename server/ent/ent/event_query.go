...
type EventQuery struct {
	...
	withRegistrations *EventRegistrationQuery
	withAgenda        *AgendaQuery
	withLocation      *AddressQuery
	withFKs           bool
...
func (_q *EventQuery) QueryProgram() *ProgramQuery {
...
func (_q *EventQuery) QueryCreatedBy() *UserQuery {
...
func (_q *EventQuery) QueryRegistrations() *EventRegistrationQuery {
...
func (_q *EventQuery) QueryAgenda() *AgendaQuery {
...
func (_q *EventQuery) QueryLocation() *AddressQuery {
...
func (_q *EventQuery) WithProgram(opts ...func(*ProgramQuery)) *EventQuery {
	query := (&ProgramClient{config: _q.config}).Query()
...
func (_q *EventQuery) WithCreatedBy(opts ...func(*UserQuery)) *EventQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) WithRegistrations(opts ...func(*EventRegistrationQuery)) *EventQuery {
	query := (&EventRegistrationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) WithAgenda(opts ...func(*AgendaQuery)) *EventQuery {
	query := (&AgendaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) WithLocation(opts ...func(*AddressQuery)) *EventQuery {
	query := (&AddressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if _q.withProgram != nil || _q.withCreatedBy != nil || _q.withLocation != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withProgram; query != nil {
		if err := _q.loadProgram(ctx, query, nodes, nil,
			func(n *Event, e *Program) { n.Edges.Program = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withRegistrations; query != nil {
		if err := _q.loadRegistrations(ctx, query, nodes,
			func(n *Event) { n.Edges.Registrations = []*EventRegistration{} },
			func(n *Event, e *EventRegistration) { n.Edges.Registrations = append(n.Edges.Registrations, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAgenda; query != nil {
		if err := _q.loadAgenda(ctx, query, nodes, nil,
			func(n *Event, e *Agenda) { n.Edges.Agenda = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withLocation; query != nil {
		if err := _q.loadLocation(ctx, query, nodes, nil,
			func(n *Event, e *Address) { n.Edges.Location = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadProgram(ctx context.Context, query *ProgramQuery, nodes []*Event, init func(*Event), assign func(*Event, *Program)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].program_events == nil {
			continue
		}
		fk := *nodes[i].program_events
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "program_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EventQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
...
func (_q *EventQuery) loadRegistrations(ctx context.Context, query *EventRegistrationQuery, nodes []*Event, init func(*Event), assign func(*Event, *EventRegistration)) error {
	...
	query.Where(predicate.EventRegistration(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.RegistrationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.event_registrations
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_registrations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "event_registrations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *EventQuery) loadAgenda(ctx context.Context, query *AgendaQuery, nodes []*Event, init func(*Event), assign func(*Event, *Agenda)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Agenda(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.AgendaColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.event_agenda
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_agenda" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "event_agenda" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *EventQuery) loadLocation(ctx context.Context, query *AddressQuery, nodes []*Event, init func(*Event), assign func(*Event, *Address)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].event_location == nil {
			continue
		}
		fk := *nodes[i].event_location
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "event_location" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
