...
func (_q *ConversationParticipantQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *ConversationParticipantQuery) FirstIDX(ctx context.Context) int {
...
func (_q *ConversationParticipantQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *ConversationParticipantQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *ConversationParticipantQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *ConversationParticipantQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(conversationparticipant.FieldLastReadAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ConversationParticipant.Query().
//		Select(conversationparticipant.FieldLastReadAt).
//		Scan(ctx, &v)
...
func (_q *ConversationParticipantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ConversationParticipant, error) {
	...
	if _q.withConversation != nil || _q.withUser != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *ConversationParticipantQuery) loadConversation(ctx context.Context, query *ConversationQuery, nodes []*ConversationParticipant, init func(*ConversationParticipant), assign func(*ConversationParticipant, *Conversation)) error {
	...
	nodeids := make(map[int][]*ConversationParticipant)
	for i := range nodes {
		if nodes[i].conversation_participants == nil {
			continue
		}
		fk := *nodes[i].conversation_participants
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "conversation_participants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ConversationParticipantQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*ConversationParticipant, init func(*ConversationParticipant), assign func(*ConversationParticipant, *User)) error {
	...
	nodeids := make(map[int][]*ConversationParticipant)
	for i := range nodes {
		if nodes[i].user_conversation_links == nil {
			continue
		}
		fk := *nodes[i].user_conversation_links
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_conversation_links" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ConversationParticipantQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(conversationparticipant.Table, conversationparticipant.Columns, sqlgraph.NewFieldSpec(conversationparticipant.FieldID, field.TypeInt))
	_spec.From = _q.sql
