...
func (_q *SkillQuery) QueryUsers() *UserSkillQuery {
...
func (_q *SkillQuery) WithUsers(opts ...func(*UserSkillQuery)) *SkillQuery {
	query := (&UserSkillClient{config: _q.config}).Query()
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withUsers; query != nil {
		if err := _q.loadUsers(ctx, query, nodes,
			func(n *Skill) { n.Edges.Users = []*UserSkill{} },
			func(n *Skill, e *UserSkill) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SkillQuery) loadUsers(ctx context.Context, query *UserSkillQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *UserSkill)) error {
	...
	query.Where(predicate.UserSkill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.UsersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.skill_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "skill_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "skill_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
