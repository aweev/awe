...
type JobQuery struct {
	...
	withDependents *JobQuery
	withDependsOn  *JobQuery
	withReports    *GeneratedReportQuery
	withFKs        bool
...
func (_q *JobQuery) QueryUser() *UserQuery {
...
func (_q *JobQuery) QueryDependents() *JobQuery {
...
func (_q *JobQuery) QueryDependsOn() *JobQuery {
	query := (&JobClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *JobQuery) QueryReports() *GeneratedReportQuery {
...
func (_q *JobQuery) WithUser(opts ...func(*UserQuery)) *JobQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *JobQuery) WithDependents(opts ...func(*JobQuery)) *JobQuery {
	query := (&JobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) WithDependsOn(opts ...func(*JobQuery)) *JobQuery {
	query := (&JobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) WithReports(opts ...func(*GeneratedReportQuery)) *JobQuery {
	query := (&GeneratedReportClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Job, error) {
	...
	if _q.withUser != nil || _q.withDependsOn != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withDependents; query != nil {
		if err := _q.loadDependents(ctx, query, nodes,
			func(n *Job) { n.Edges.Dependents = []*Job{} },
			func(n *Job, e *Job) { n.Edges.Dependents = append(n.Edges.Dependents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDependsOn; query != nil {
		if err := _q.loadDependsOn(ctx, query, nodes, nil,
			func(n *Job, e *Job) { n.Edges.DependsOn = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withReports; query != nil {
		if err := _q.loadReports(ctx, query, nodes,
			func(n *Job) { n.Edges.Reports = []*GeneratedReport{} },
			func(n *Job, e *GeneratedReport) { n.Edges.Reports = append(n.Edges.Reports, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *JobQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Job, init func(*Job), assign func(*Job, *User)) error {
...
func (_q *JobQuery) loadLogs(ctx context.Context, query *JobLogQuery, nodes []*Job, init func(*Job), assign func(*Job, *JobLog)) error {
	...
	for _, n := range neighbors {
		fk := n.job_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "job_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "job_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *JobQuery) loadDependents(ctx context.Context, query *JobQuery, nodes []*Job, init func(*Job), assign func(*Job, *Job)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Job)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Job(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(job.DependentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.job_depends_on
		if fk == nil {
			return fmt.Errorf(`foreign-key "job_depends_on" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "job_depends_on" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *JobQuery) loadDependsOn(ctx context.Context, query *JobQuery, nodes []*Job, init func(*Job), assign func(*Job, *Job)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Job)
	for i := range nodes {
		if nodes[i].job_depends_on == nil {
			continue
		}
		fk := *nodes[i].job_depends_on
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "job_depends_on" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *JobQuery) loadReports(ctx context.Context, query *GeneratedReportQuery, nodes []*Job, init func(*Job), assign func(*Job, *GeneratedReport)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Job)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.GeneratedReport(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(job.ReportsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.job_reports
		if fk == nil {
			return fmt.Errorf(`foreign-key "job_reports" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "job_reports" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
