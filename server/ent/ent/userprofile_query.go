...
func (_q *UserProfileQuery) QueryAssignments() *VolunteerAssignmentQuery {
...
func (_q *UserProfileQuery) QuerySkills() *UserSkillQuery {
	query := (&UserSkillClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserProfileQuery) QueryAddress() *AddressQuery {
...
func (_q *UserProfileQuery) WithAssignments(opts ...func(*VolunteerAssignmentQuery)) *UserProfileQuery {
	query := (&VolunteerAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserProfileQuery) WithSkills(opts ...func(*UserSkillQuery)) *UserProfileQuery {
	query := (&UserSkillClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserProfileQuery) WithAddress(opts ...func(*AddressQuery)) *UserProfileQuery {
	query := (&AddressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(userprofile.FieldFirstName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.UserProfile.Query().
//		Select(userprofile.FieldFirstName).
//		Scan(ctx, &v)
...
func (_q *UserProfileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserProfile, error) {
	...
	if _q.withUser != nil || _q.withAddress != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *UserProfile) { n.Edges.Assignments = []*VolunteerAssignment{} },
			func(n *UserProfile, e *VolunteerAssignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSkills; query != nil {
		if err := _q.loadSkills(ctx, query, nodes,
			func(n *UserProfile) { n.Edges.Skills = []*UserSkill{} },
			func(n *UserProfile, e *UserSkill) { n.Edges.Skills = append(n.Edges.Skills, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAddress; query != nil {
		if err := _q.loadAddress(ctx, query, nodes, nil,
			func(n *UserProfile, e *Address) { n.Edges.Address = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserProfileQuery) loadAssignments(ctx context.Context, query *VolunteerAssignmentQuery, nodes []*UserProfile, init func(*UserProfile), assign func(*UserProfile, *VolunteerAssignment)) error {
	...
	query.Where(predicate.VolunteerAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(userprofile.AssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_profile_assignments
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_profile_assignments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_profile_assignments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserProfileQuery) loadSkills(ctx context.Context, query *UserSkillQuery, nodes []*UserProfile, init func(*UserProfile), assign func(*UserProfile, *UserSkill)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*UserProfile)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.UserSkill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(userprofile.SkillsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_profile_skills
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_profile_skills" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_profile_skills" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserProfileQuery) loadAddress(ctx context.Context, query *AddressQuery, nodes []*UserProfile, init func(*UserProfile), assign func(*UserProfile, *Address)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserProfile)
	for i := range nodes {
		if nodes[i].user_profile_address == nil {
			continue
		}
		fk := *nodes[i].user_profile_address
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_profile_address" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
