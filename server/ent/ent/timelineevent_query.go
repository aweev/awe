...
func (_q *TimelineEventQuery) QueryBlock() *BlockTimelineQuery {
...
func (_q *TimelineEventQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *TimelineEventQuery) FirstIDX(ctx context.Context) int {
...
func (_q *TimelineEventQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *TimelineEventQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *TimelineEventQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *TimelineEventQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *TimelineEventQuery) WithBlock(opts ...func(*BlockTimelineQuery)) *TimelineEventQuery {
	query := (&BlockTimelineClient{config: _q.config}).Query()
...
//		GroupBy(timelineevent.FieldOrder).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.TimelineEvent.Query().
//		Select(timelineevent.FieldOrder).
//		Scan(ctx, &v)
...
func (_q *TimelineEventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TimelineEvent, error) {
	...
	if query := _q.withBlock; query != nil {
		if err := _q.loadBlock(ctx, query, nodes, nil,
			func(n *TimelineEvent, e *BlockTimeline) { n.Edges.Block = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TimelineEventQuery) loadBlock(ctx context.Context, query *BlockTimelineQuery, nodes []*TimelineEvent, init func(*TimelineEvent), assign func(*TimelineEvent, *BlockTimeline)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TimelineEvent)
	for i := range nodes {
		if nodes[i].block_timeline_events == nil {
			continue
		}
		fk := *nodes[i].block_timeline_events
	...
	query.Where(blocktimeline.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "block_timeline_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TimelineEventQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(timelineevent.Table, timelineevent.Columns, sqlgraph.NewFieldSpec(timelineevent.FieldID, field.TypeInt))
	_spec.From = _q.sql
