...
func (_q *GrantQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *GrantQuery) QueryProgram() *ProgramQuery {
...
func (_q *GrantQuery) QueryReports() *GrantReportQuery {
	query := (&GrantReportClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GrantQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *GrantQuery) FirstIDX(ctx context.Context) int {
...
func (_q *GrantQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *GrantQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *GrantQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *GrantQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *GrantQuery) WithOrganization(opts ...func(*OrganizationQuery)) *GrantQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
...
func (_q *GrantQuery) WithProgram(opts ...func(*ProgramQuery)) *GrantQuery {
	query := (&ProgramClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GrantQuery) WithReports(opts ...func(*GrantReportQuery)) *GrantQuery {
	query := (&GrantReportClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GrantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Grant, error) {
	...
	if _q.withOrganization != nil || _q.withProgram != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *Grant, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProgram; query != nil {
		if err := _q.loadProgram(ctx, query, nodes, nil,
			func(n *Grant, e *Program) { n.Edges.Program = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withReports; query != nil {
		if err := _q.loadReports(ctx, query, nodes,
			func(n *Grant) { n.Edges.Reports = []*GrantReport{} },
			func(n *Grant, e *GrantReport) { n.Edges.Reports = append(n.Edges.Reports, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GrantQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *Organization)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Grant)
	for i := range nodes {
		if nodes[i].organization_grants == nil {
			continue
		}
		fk := *nodes[i].organization_grants
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_grants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *GrantQuery) loadProgram(ctx context.Context, query *ProgramQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *Program)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Grant)
	for i := range nodes {
		if nodes[i].program_grants == nil {
			continue
		}
		fk := *nodes[i].program_grants
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "program_grants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *GrantQuery) loadReports(ctx context.Context, query *GrantReportQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *GrantReport)) error {
	...
	query.withFKs = true
	query.Where(predicate.GrantReport(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(grant.ReportsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.grant_reports
		if fk == nil {
			return fmt.Errorf(`foreign-key "grant_reports" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "grant_reports" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
